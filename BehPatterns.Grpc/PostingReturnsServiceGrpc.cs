...
// Псевдоним для proto-сгенерированных типов, чтобы не конфликтовали
// с внутренними IRequest-сообщениями с такими же именами.
using Proto = BehPatterns.Grpc.PostingReturns;

namespace BehPatterns.Grpc
{
    // gRPC-контроллер. Тонкий: маппит proto -> IRequest, отдаёт медиатору,
    // маппит ответ обратно в proto. Не ссылается ни на один handler.
